...
prizeApi.prototype = {
  ...
  list: function (callback) {
      var that = this;
      var args = [];
      that.getURLData(that.apiURL['list'], args, callback);
  },
 ...
 *
 * @param int           qid         [有奖问答id]
 * @param int           success_token         [小程序分享成功后返回的标识]
 * @param string        callback    [回调方法]
 * 
 */
  isshare: function (qid,success_token,callback) {
      var that = this;
      var args = { qid: qid, success_token: success_token};
      that.postURLData(that.apiURL['isshare'], args, callback);
  },
